...
class JSONParseEvaluator:
    ...
    def cal_f1(self, preds: List[dict], answers: List[dict]):
        ...
        for pred, answer in zip(preds, answers):
            pred, answer = self.flatten(self.normalize_dict(self.construct_dict_from_raw(pred))), self.flatten(self.normalize_dict(self.construct_dict_from_raw(answer)))
            for field in pred:
                if field in answer:
                    total_tp += 1
                    answer.remove(field)
    ...
    def construct_dict_from_raw(self, raw):
        output_dict = dict()
        for phrase in raw:
            if not isinstance(phrase, dict):
                continue
            label_key = "ner_tag" if "ner_tag" in phrase.keys() else "label"
            label = phrase[label_key]
            if label == "other":
                continue
            if label not in output_dict.keys() and (phrase['is_key'] or phrase['is_value']):
                output_dict[label] = {
                    "key": "",
                    "value": "",
                }
            ## Modify to merge content NOT replace
            if phrase['is_key']:
                if not output_dict[label]['key']:
                    output_dict[label]['key'] = phrase['text']
                else:
                    output_dict[label]['key'] += " " + phrase['text']
            elif phrase['is_value']:
                if not output_dict[label]['value']:
                    output_dict[label]['value'] = phrase['text']
                else:
                    output_dict[label]['value'] += " " + phrase['text']
    ...
    def cal_acc(self, pred: dict, answer: dict):
        ...
        """
        # pred = self.construct_tree_from_dict(self.normalize_dict(self.construct_dict_from_raw(pred)))
        # answer = self.construct_tree_from_dict(self.normalize_dict(self.construct_dict_from_raw(answer)))
